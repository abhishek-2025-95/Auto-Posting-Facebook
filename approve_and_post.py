#!/usr/bin/env python3
"""
Approve and post the professional subtitle video
"""
...
def find_latest_professional_video():
    """Find the latest professional subtitle video"""
    video_files = glob.glob("professional_subtitle_video_*.mp4")
    
    if not video_files:
        print("No professional subtitle videos found.")
        return None
...
def approve_and_post():
    """Approve and post the professional subtitle video"""
    print("="*60)
    print("APPROVE AND POST PROFESSIONAL SUBTITLE VIDEO")
    print("="*60)
    
    # Find latest professional video
    video_file = find_latest_professional_video()
    
    if not video_file:
        print("No professional subtitle video found.")
        return False
    
    print(f"Found professional video: {video_file}")
    ...
    confirm = input("\nDo you want to post this professional subtitle video to Facebook? (y/n): ").lower().strip()
    ...
    if success:
        print("\n" + "="*60)
        print("SUCCESS! PROFESSIONAL VIDEO POSTED!")
        print("="*60)
