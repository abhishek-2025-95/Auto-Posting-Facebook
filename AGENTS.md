...
2. **Images (choose one)**  
   - **A — Cursor Background Agents API (hands-free):** In **`.env`**, set **`CURSOR_API_KEY`** (Cursor Dashboard → Integrations), **`CURSOR_BACKGROUND_AGENT_AUTO=1`**, and **`CURSOR_BACKGROUND_AGENT_REPO=https://github.com/ORG/REPO`** (or use **`git remote origin`**). **Required:** in the same Dashboard, connect **GitHub** and grant the **Cursor GitHub App** access to that repository; otherwise the API returns errors like *“Failed to verify existence of branch ‘main’”*. Optional: leave **`CURSOR_BACKGROUND_AGENT_REF=`** empty in **`.env`** to omit `ref` (uses GitHub default) once access works.  
